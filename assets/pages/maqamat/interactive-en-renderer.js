// English interactive maqam renderer: sidebar, page shell, SVG staff, note keys, and page UI updates.
...
(function () {
  ...
  function renderPageShell() {
    ...
    const latinDisplayName = getLatinDisplayNameForTonicEn(state.maqamId, state.tonic);
    const tonicLabel = getTonicLabelEn(state.tonic);
    ...
    root.innerHTML = `
      ...
      <section class="maqam-body">
        ...
        <div class="sec-title">Maqam details</div>
  ...
  function renderTonicSelector() {
    ...
    if (!container) return;
    const tonics = getInteractiveTonicsForMaqamEn(ns.state.maqamId);
    container.innerHTML = tonics.map(tonic => `<button class="tonic-btn ${tonic === ns.state.tonic ? 'active' : ''}" data-tonic="${tonic}">${getTonicLabelEn(tonic)}</button>`).join('');
  ...
  function renderInfoGrid() {
    ...
    const model = ns.engine.MAQAM_MODELS[mapEnMaqamIdToInteractive(ns.state.maqamId)];
    const displayName = ns.engine.getDisplayNameForTonicSafe(ns.state.maqamId, ns.state.tonic);
    const direction = ns.engine.getMaqamDirection(mapEnMaqamIdToInteractive(ns.state.maqamId));

    grid.innerHTML = `
      <div class="info-card"><div class="info-label">Displayed name</div><div class="info-value gold">${displayName}</div></div>
      <div class="info-card"><div class="info-label">Current tonic</div><div class="info-value">${getTonicLabelEn(ns.state.tonic)}</div></div>
      <div class="info-card"><div class="info-label">Base model</div><div class="info-value" dir="ltr">${model ? model.base_spelling.join(' - ') : ''}</div></div>
      <div class="info-card"><div class="info-label">Intervals</div><div class="info-value" dir="ltr">${model ? model.intervals.join(' / ') : ''}</div></div>
      <div class="info-card"><div class="info-label">Direction</div><div class="info-value">${direction === 'descending' ? 'Descending' : 'Ascending'}</div></div>
    `;
  }
  ...
  function updateDisplayedName() {
    ...
    const displayName = ns.engine.getDisplayNameForTonicSafe(ns.state.maqamId, ns.state.tonic);
    const latinDisplayName = getLatinDisplayNameForTonicEn(ns.state.maqamId, ns.state.tonic);
    const tonicLabel = getTonicLabelEn(ns.state.tonic);
